...
exports.listFoldersFromPath = function (folderPath, opts) {
  ...
  folders = folders.filter( (folder) => {
    ...
    let isDirectory = fs.statSync(path.join(folderPath, folder)).isDirectory()
...
exports.listFilesFromPath = function (folderPath, opts) {
  ...
  files = files.filter( (file) => {
    ...
    // Check if is a file
    let isFile = fs.statSync(path.join(folderPath, file)).isFile()

    // Check if one of the extensions
    let isValidExtension = opts.extensions.indexOf(extension) >= 0
