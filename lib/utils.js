 ...
 * Opts available:
 *   .skip: [String]:
 *     An array of patterns to match and check if it should skip the file
 *
 *   .canMerge [Boolean]: 
...
exports.listFilesFromPath = function (folderPath, opts) {
  ...
  files = files.filter( (file) => {
    ...
    // Check if matches pattern to skip
    var shouldSkipFile = opts.skip.find( (pattern) => pattern.test(file))
    if (shouldSkipFile)
      return false
...
exports.pathNodesForFile = function (fileName, opts) {
  ...
  fileName = exports.fileName(fileName, opts)
  fileName = exports.sanitizeName(fileName, opts)
...
/*
 * This will process and return an absolute path given an relative or absolute path.
 * This function must be called directly in the function the user called with the relative path.
 */
exports.absolutePathFor = function (folder) {
  // Check if is a relative path
  var start = folder.substring(0, 2);
  if (start === './' || start === '..') {
    var callerPathFolder = path.dirname(callPath(2))
    return path.join(callerPathFolder, folder)
  }
